...
const GameBoard = () => {
  ...
  const handleChoice = (playerId: number, roundIndex: number, choice: Choice) => {
    setPlayers(prev =>
      prev.map(player =>
        player.id === playerId
          ? {
  ...
  const determineWinner = (gamePlayers: Player[], roundIndex: number): RoundResult => {
    ...
    const currentChoices = activePlayers.map(p => p.choices[roundIndex]);
  ...
  const startGame = () => {
    // 全プレイヤーの全ラウンドがnullでないことを確認
    const allPlayersReady = players.every(p => p.choices.every(choice => choice !== null));
    if (!allPlayersReady) {
      toast.error("All players need to select their moves for all rounds!");
      return;
    }
        ...
        <div className="space-y-8">
          ...
          {Array.from({ length: TOTAL_ROUNDS }, (_, i) => (
            <ChoiceSelector
              ...
              onChoiceSelect={(round, choice) => handleChoice(1, round, choice)}
            />
          ))}
          {/* Player2's UI */}
          {Array.from({ length: TOTAL_ROUNDS }, (_, i) => (
            <ChoiceSelector
              key={`player2-round${i}`}
              roundNumber={i + 1}
              selectedChoices={players[1].choices}
              onChoiceSelect={(round, choice) => handleChoice(2, round, choice)}
            />
          ))}
          {/* Player3's UI */}
          {Array.from({ length: TOTAL_ROUNDS }, (_, i) => (
            <ChoiceSelector
              key={`player3-round${i}`}
              roundNumber={i + 1}
              selectedChoices={players[2].choices}
              onChoiceSelect={(round, choice) => handleChoice(3, round, choice)}
            />
          ))}
        </div>
      )}
